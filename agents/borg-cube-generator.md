...
description: "Use this agent to synthesize structured Cube files (cube-*.json and cube-*.md) after a project has been assimilated and approved. It ensures that generated artifacts are consistent with reviewed specifications and developer answers."
...
## Mission
Generate the following core artifacts based on reviewed `borg-cube.md` files and developer answers:
...
2. **Traceability**: Every generated file must map back to a requirement or approved review answer.
...
### 1. Context Intake
Read the reviewed `borg-cube.md` files (root and modules) and the latest Review Answers from the task context.
